...
while SIZE > 10:
    ...
    for vel in range(0, opakovani):
        ...
        if SIZE > 2 and chodba == True:
            barva = hexCodeFromRgb(random.randint(0, int(SIZE / 3)), random.randint(0, int(SIZE / 3)),
                                   random.randint(0, int(SIZE / 3)))
            C.create_rectangle(Hx, Hy, Hx + SIZE, Hy + SIZE, fill=barva)
        elif SIZE > 2:
            barva = hexCodeFromRgb(random.randint(100 - int(SIZE / 5), 200), random.randint(100 - int(SIZE / 5), 200),
                                   random.randint(100 - int(SIZE / 5), 200))
            C.create_rectangle(Hx, Hy, Hx + SIZE, Hy + SIZE, fill=barva)
        else:
            break
